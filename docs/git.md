...
Состояние индекса можно узнать в любой момент с помощью коианды:
...
*Игнорируемые* явно объявленые файлы и деректории в .gitignore 

*Неотслеживаемые* файлы которые не добавлены в индекс и отсутвуют в .gitignore
...
Производится фиксация изменений добавленых в индекс.
...
Команда удаляет файлы из индекса и из каталога. Либо удаляет файлы только  из индекса. Не может удалить файлы только с диска(для этого нужно использовать системную команду), т.е. команда работает только с файлами добавлеными в репозиторий.
...
Удалит даже отслеживаемый модифицированный файл не добавленый в индекс.

Для востановления файла удаленного командой ```git rm``` можно испоьзовать механизм востановления файлов:
...
```\#file.txt``` -- если название фацла начинается с решетки

```file.txt``` -- имена файлов полном виде

```dir/``` -- каталог отмечатся следующим образом

```*``` -- заменяет любые символы

Например:

```*.txt``` -- игнорировать все файлы смрасширением .txt

```!``` --- задает исключение для общего правила заданого с помощью предшествующего правила.
...
exclude -- содержит шаблоны как и в .gitignore. Позволяет игнорировать специфичные файлы хаоактерные для операционной системы или ПО, но в .gitignore репозитория отмечать файлы характерные только для его специфики. 
