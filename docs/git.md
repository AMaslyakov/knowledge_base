...
### ```git diff``` и диапазонны фиксаций

Эквивалентны следующие комнды:

```bash
git diff main bug/pr1
git diff main..bug/pr1
```
Диапазон в git diff игнроируется. 
Будет показана разница только между двумя коммитами ```main bug/pr1```.
